...
function object(characteristics) {
    ...
    // cx - xcoordinate at the CENTER
    // cy - ycoordinate at the CENTER
    ...
    this.prevY = this.y; //UNUSED IGNORE
    this.prevX = this.x; //UNUSED IGNORE
    
    if (characteristics.shape == null) {
        characteristics.shape = "circle";
    }

    
    //object motions
    this.gravity = 10.0/ 3.0 / 25.0;
    this.currentVeloc = 0.0; //velocity during interaction
    this.walkingSpeed = 5; //speed when using arrow key/no other interactions
    this.passiveSpeed = 0; //normal speed when not interacted 
    ...
    this.redraw = function () {
        ...
        // if characteristics.shape == circle ->
        //ctx.beginPath();
        // this.ctx.arc(x, y, h/2 (rad), 0, 2* Math.PI)
        //this.ctx.stroke()
        console.log(characteristics.circle)
    ...
    window.addEventListener('keydown', function(event) {
        ...
        if (event.code == "Space") {
            //objects.protagonist.keyPressed.spacebar.num+=1; 
            state.spacebar.pressed = true;
            objects.protagonist.keyPressed.spacebar.num+=1;
            //console.log(objects.protagonist.keyPressed.spacebar.num);
            // if ((event.code === "Space") && (state.spacebar.num == 2)) {
      ...
      //  console.log(objects.protagonist.keyPressed.spacebar.num);
        ...
        window.addEventListener('keyup', function(event) {   
            ...
            if (event.code == "Space") {state.spacebar.pressed = false;}
        ...
        this.isAirborne = function() {
            ...
            for (let i in objects) {
                ...
                !(objects[i]==this)) { // don't look at obj 1
                   ...
                   // (this.keyPressed.spacebar.pressed.num == 1) ? console.log(objects[i]) : 0;
            ...
            if (this.keyPressed.up) {
                            ...
                            if (this.keyPressed.right) {
                                ...
                                //     if (this.interactions.xcoll.fromWhere == "right") {
                                    //         if (this.interaction.ycoll.object.moveable == true) {
                                     ...
                                     //   console.log(this.keyPressed.spacebar.num);
                                        

                                        //if you only jumped 0 or 1, you can jump
                                        //else, no more jumping (FOR NOW)
                                        spacebarPressed = this.keyPressed.spacebar.pressed;
                                    if (spacebarPressed) {
                                        if (this.keyPressed.spacebar.num == 1) {
                                            this.motions.jumping = true;
                                                                                     
                                        } else if (this.keyPressed.spacebar.num == 2) {
                                            this.motions.jumping = true;
                                                // console.log("c")
                                           
                                        } else {
                                            this.motions.jumping = false;
                                         //   objects.protagonist.keyPressed.spacebar.num = 0;
                                        }
                                    } else {
                                        this.motions.jumping = false;
                                      //  objects.protagonist.keyPressed.spacebar.num = 0;
                        ...
                        this.coll();
                                    ...
                                    if ((state = this.isAirborne()) == true) {
                                        ...
                                        // console.log(state)
                                    //    console.log(objects.protagonist.keyPressed.spacebar.num, 'air'); 
                                        if (state.y > this.y) {
                                            this.y = state.y - this.height; //reset height/end jump
                                        }

                                        if (this.jumpState) {
                                            this.keyPressed.spacebar.pressed.num = 0;
                                        }
                                        //  this.jumpState = false;
                                        // this.keyPressed.dblspacebar = false;
                                        

                                        //where to put this block of code?????!?!?
                                         //reset spacebar count
                                       
                                        this.currentVeloc = 0.0;
                                   // console.log("checkgerg");
                                        //console.log("check")
                                    ...
                                    for (let i in objects) { //do a loop to find all of the connected objects, and then move all of them together.
                                        if (objects[i].motions.right) {objects[i].x+=1}
                                         ...
                                         if (objects[i].motions.jumping) {
                                            this.currentVeloc = this.JumpVeloc;  
                                            objects.protagonist.y -= objects.protagonist.currentVeloc;
                                            this.jumpState = true;
                                            // objects.protagonist.motions.jumping = false;
                                            // objects.protagonist.spacebar.num--;
                                            // console.log("check")
                                        }
